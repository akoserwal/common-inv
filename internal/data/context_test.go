...
var (
	...
	c2 = context.WithValue(c, TransactionKey, &tx)
	f  = func() {}
)

func Test_NewContext(t *testing.T) {
	transaction, err := mockConn.newTransaction()
	c3 := context.WithValue(c, TransactionIDkey, transaction.txid)
	c3 = context.WithValue(c3, TransactionKey, transaction)
	type fields struct {
		c *ConnectionFactory
	}
	type args struct {
...
func Test_FromContext(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				err: fmt.Errorf("could not retrieve transaction from context"),
			},
			want:    nil,
			wantErr: true,
		},
...
func Test_MarkForRollback(t *testing.T) {
	...
	c4 := context.WithValue(c, TransactionKey, &txF)
	type args struct {
		ctx context.Context
	}
	tests := []struct {
	...
	for _, testcase := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			g := gomega.NewWithT(t)
			t.Parallel()
			transaction, ok := tt.args.ctx.Value(TransactionKey).(*txFactory)
			if ok {
				g.Expect(transaction.rollbackFlag).To(gomega.Equal(false))
				MarkForRollback(tt.args.ctx, nil)
				transaction = tt.args.ctx.Value(TransactionKey).(*txFactory)
				g.Expect(transaction.rollbackFlag).To(gomega.Equal(true))
			} else {
				MarkForRollback(tt.args.ctx, fmt.Errorf(""))
			}
		})
	}
}
